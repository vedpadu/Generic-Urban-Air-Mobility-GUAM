...
for i = 1:21
    if length(Out.trim.WH) > 1 
        ii = ii0(i);
        XU0(i,1) = interp2(Out.trim.WH, Out.trim.UH, squeeze(Out.trim.XU0_interp(ii,:,:)), wbar0, ubar0,'linear',Out.trim.XU0_interp(ii,1,2));
    else
        XU0(i,1) = interp1(Out.trim.UH, Out.trim.XU0_interp(ii,:), ubar0, 'linear','extrap');
    end
end
Out.XU0 = XU0;
